...
- **Ce que la machine apprécie, et ce qu'elle n'apprécie pas.** « Fruit » a deux sens : *mûr* — formellement complet, plus rien à instruire, n'attend qu'un verdict — et *bon*, c'est-à-dire vrai. **La machine ne juge que le premier** (Cmd 12). Un rang élevé dans cette récolte ne dit rien de la validité métaphysique d'une thèse ; il dit qu'elle est prête à être tranchée.
- **Assiette** : 58 fiches du Registre — 17 `traditionnel`, 1 `contre-traditionnel`, 1 `adopte` (hors vocabulaire du Sceau), 41 `speculatif`, ces dernières formant la récolte proprement dite.
- **Le triage ne rend pas 41 lignes, il rend des dossiers.** Rendre l'assiette fiche par fiche aurait reproduit le problème énoncé. Les fiches interdépendantes sont groupées pour se traiter d'un seul tenant : dossier kabbalistique ; **dossier de Qâf du 2026-08-30, dont l'ordre est contraint** — `2026-08-30_chute-realisation-deux-aspects-de-qaf` porte la thèse et doit être tranchée avant `khafd-raf` et `gens-de-al-araf`, que son §4 nomme comme dépendantes (vérifié sur le texte de la fiche, non supposé) ; six verdicts d'un seul mot ; `2026-08-25_gizeh-degre-24` mûr **parce qu'il ne mûrira pas davantage** — la chaîne documentaire est close, aucun texte ne nomme Gizeh, ce qui reste est un verdict et non une lecture ; `kamon` jumelé à `sashimono` bloqué ; trois partiels ; neuf en dépendance externe (ouvrages possédés non photographiés) ; la cohorte du 2026-06-20 ; deux qui attendent une thèse et non un verdict.
- **Trois anomalies formelles signalées, non corrigées** (Cmd 12 — les relever est du ressort de la machine, les redresser non) : `2026-08-11_trois-territoires-inacheve` porte `status: adopte`, hors des cinq valeurs du Sceau, avec un `verdict_date` ; `2026-08-20_traite-emanation-gauche` et `2026-08-20_etat-lieux-kabbale-depot` sont `type: discernement` **sans aucun bloc 🔍**, contrairement à l'action EXAMEN DE DISCERNEMENT ; `2026-07-05_correspondances-fonctions-initiatiques-entreprise` a son volet (b) clos le 2026-07-14 puis rouvert le 2026-08-15, son volet (a) jamais instruit.
- **Ces anomalies ont servi de matière au contrôle mécanique ajouté le même jour** : la première a motivé le code C7 de l'étanchéité inversée (un statut hors vocabulaire rend le contrôle indécidable, et le vérificateur le dit au lieu de supposer la fiche close) ; les deux fiches sans bloc 🔍 ont écarté la définition de « non tranché » par le corps au profit du cartouche. Voir `atelier/annales.md`, entrées du 2026-09-10.
- **Livrable au sas, non versé aux circuits** : `_inbox/2026-09-10_recolte-discernement-etat-maturite.md`, 174 lignes. Sa place en `_inbox/` la tient hors du périmètre du vérificateur ; versionnée sur demande de Sidy pour lecture dans Obsidian, elle ne franchit pas le sas pour autant.
- **Aucun verdict rendu.** La récolte compile, source et structure ; elle ne tranche pas (Cmd 12).
- **Commit** : 16752a6
...
- **Le chiffre de 37 n'était pas reproductible, et c'est le premier fait de cette passe.** Il figurait dans mon propre rapport de clôture (« 66 des 103 fiches sans candidat annotable ») mais provenait d'une fenêtre antérieure, et le critère qui l'avait produit n'a pas pu être retrouvé. Plutôt que de rétro-ajuster un critère pour retomber sur la phrase — ce qui eût été **ajuster la mesure au propos** —, le critère a été réénoncé explicitement et la mesure refaite : un candidat est un `tags:` ou une **cible de wikilink de la fiche** (condition D1), retrouvé dans le corps **avec ses diacritiques**, hors titre, wikilink, code et bloc cité. **104 fiches, 5 déjà annotées, 41 sans candidat, 58 annotables.**
- **La correspondance se fait sur la forme normalisée, non sur le slug nu** : `ibn-arabi` trouve « Ibn ʿArabī », `istidad` trouve « istiʿdād », `alam-al-mithal` trouve « ʿālam al-mithāl ». Une correspondance littérale sur le slug les aurait tous manqués — le dépôt écrit sa prose avec les diacritiques que le slug a décapés.
- **163 poses proposées, 136 retenues, 27 rejetées** (21 clés), chacune avec son motif. Rejetés : les **noms communs français que le repli ASCII rend homographes d'un tag** — `métaphysique` (×5), `sainteté` (glose de *walāya*), `manifestation`, `résurrection`, `polarité`, `rotation`, `isthme` (glose de *barzakh*), `déluge`, `aurore`, `discrimination`, `cosmologie`, `zodiaque`, `réincarnation`, `prophétologie` ; `mystères`, capté dans un **en-tête de table** (une étiquette de colonne n'est pas une occurrence du terme) ; les syntagmes descriptifs (`tradition juive`, `création artificielle`, `cérémonie collective`) ; l'adjectif `lurianique` ; et `wiki-manifest`, artefact d'infrastructure sans portée doctrinale.
- **Classement des 67 clés retenues** — c'est la contribution propre de la machine (Cmd 12 : sur la forme, le modèle se prononce) : **35 `<dfn>`** termes techniques, `data-tradition` pris **mécaniquement** du `tradition_cadre` de la fiche, jamais deviné (*barzakh, walāya, taṣawwuf, tawakkul, khalwa, dhikr, silsila, miʿrāj, ʿilm al-ḥurūf, ʿilm al-nujūm, nâdîs, sushumnâ, piṅgalā, guṇas, afrâd, ḥasb, asbāb, tikun, Ibur, Khātim, Quṭbiyya*…) ; **14 `autorite`** (*Ibn ʿArabī* à lui seul 26 poses, *Ibn Sīrīn*, *Al-Ghazālī*, *Al-Nābulusī*, *Aḥmad al-Būnī*, *Ibn Sīnā*, *Ibn Qayyim*, *Ḥaïm Vital*, *René Guénon*, *Platon*, *Tilak*, *Gloton*, *Jīlī*, *Hamza Yusuf*) ; **7 `cycle`**, **4 `ecole`** (*Naqshbandiyya, Sāṃkhya, Vêdânta, Kabbale*), **4 `entite`** (*Poséidon, Janus, Hachiman, Sephiroth*), **2 `ouvrage`** (*Fuṣūṣ al-Ḥikam, Shams al-Maʿārif*), **1 `principe`**.
- **Pose par script déterministe, jamais à la main** (§VII, règle 1 des manifestes) : le proposeur applique les règles de placement ratifiées le jour même, écrit de la fin vers le début pour que les offsets restent valides, et remonte `updated:` (Cmd 8). Le classement vit dans un JSON séparé, relisible. **Diff de 180 lignes pour 180 : aucun texte n'a été modifié.**
- **Les règles ratifiées ont mordu d'elles-mêmes.** Dans `wahdat-al-wujud.md`, `barzakh` n'est **pas** annoté : sa seule occurrence est dans un wikilink (`*[[barzakh]]*`) — règle 1, refus D4 par anticipation. Le proposeur n'a rien eu à corriger après coup ; il n'a simplement pas posé.
- **Contrôles** — `valider-annotations.py` v1.1 : « 63 fiches annotées, 157 annotations. OK — aucune anomalie », exit 0 (D1 appariement, D2 Unicode, D3 plancher, D4 placement, D5 occurrence unique, vocabulaire clos). `verifier-invariants.py` : **1417 fichiers, 0 erreur(s), 0 avertissement(s)**. Index régénéré : **82 termes portent désormais le rôle `annotation`** contre 21 avant la passe.
- **Le régime apparié vérifié empiriquement, et non plus seulement affirmé.** Graphe régénéré : 570 nœuds, 1937 arêtes. Les **arêtes sont rigoureusement identiques**, comparées terme à terme après tri — **136 annotations n'ont créé aucun lien**. Les nœuds ne diffèrent que des `updated:` remontés. C'est la démonstration, sur pièce, que l'HTML **type** ce que le graphe voyait déjà et ne porte jamais l'existence d'un renvoi.
- **Reste ouvert (Cmd 12)** : **41 fiches** de `doctrinal/symboles/` restent sans candidat annotable — leur vocabulaire n'est ni tagué ni lié, et l'appariement D1 le leur interdit ; les 27 poses rejetées sont listées ci-dessus avec leur motif et peuvent être reprises une à une si Sidy en juge autrement ; les autres sous-dossiers de `doctrinal/` (`traditions/`, `autorites/`, `doctrines/`, `deviations/`, `etudes/`, `discernement/`) n'ont pas été touchés — la passe portait sur `symboles/` seul.
- **Commits** : 6d4f650 (les 58 fiches), 1223a39 (graphe régénéré).
...
- **Action** — création de trois fiches doctrinales et de deux planches : [[doctrinal/etudes/2026-09-08_kamon-glossaire-systematique]] (le glossaire : histoire, grammaire du tracé, morphologie des variantes, répertoire d'environ 130 motifs en huit familles, table de 60 maisons, régime d'usage, quatre voies attestées de composition), [[doctrinal/symboles/tomoe]], et [[doctrinal/discernement/2026-09-08_kamon-symbolisme-traditionnel-ou-convention-heraldique]]. L'établi de conception vit hors circuit, au label (entrée propre à `label/annales.md`).
- **Sourçage — le point le plus important de cette passe.** Le catalogue de la bibliothèque physique a été consulté **en premier lieu**, comme la discipline des sources l'impose : il ne porte **aucun ouvrage japonais, aucun traité d'héraldique, aucun répertoire de mon**. Toute la matière factuelle est de **documentation secondaire de seconde main**. Le `to-source` porte donc sur **l'étude entière**, non motif par motif, et le § IX nomme les trois ouvrages qui seuls permettraient de le lever : un répertoire japonais illustré, un ouvrage de référence occidental, un *bukan* d'époque Edo — ce dernier étant la seule source qui vaille pour les attributions de maisons, parce qu'elle est contemporaine des maisons qu'elle décrit. **Aucune fiche ne doit citer cette étude comme une autorité.**
- **Les images — ce qui a été fait et ce qui a été refusé.** Aucun fac-similé de mon n'est versé : un dessin approximatif, dans un dépôt de transmission, se transmet comme s'il était juste. À la place : le **blason verbal** de chaque motif, et **deux planches SVG générées par script déterministe** (`doctrinal/etudes/assets-kamon/`) — constructions géométriques en 16 panneaux, grammaire des variantes en 19 vignettes. Elles ne portent **que** des figures constructibles au compas et à la règle ; les motifs organiques (*kiri*, *aoi*, *fuji*, *kiku*) ne sont **délibérément pas** figurés. Le panneau 16 donne un résultat exact et non approché : les huit disques du *kuyō* tangents au disque central **et entre eux** exigent r/r_c = sin(22,5°)/(1 − sin(22,5°)) = 0,619914. Convention d'assets reprise de `assets-instrument/` (dossier `assets-` adjacent au sujet, fichiers texte donc versionnables).
- **Double contrôle (§VII)** — état d'ensemble ***kari-kumi***. Un joint approché en portance *zōsaku* (règle du compas ↔ discipline sashimono, éclairant, sans portée doctrinale) ; **trois rapprochements explicitement refusés** : *hinata*/*kage* ↔ *yin*/*yang* (c'est une échelle de formalité vestimentaire, aucune source ne pose la lecture cosmologique), *kagome* ↔ hexagramme (coïncidence de figure), mon-caractère ↔ science des lettres (le mon use du caractère comme marque nominale). La fiche *tomoe* en refuse trois autres — triskèle, *yin/yang*, tourbillon principiel. **Ces refus sont l'apport principal de la passe** : ce sont les rapprochements que le corpus offre le plus spontanément.
- **Confrontation Gizeh — conduite sur quatre points, résultat NÉGATIF, consigné comme la règle l'exige.** (1) Septénaire : le *shichiyō* est un fait de calendrier sino-indien, non une structure cosmologique — contact de nombre, non de doctrine. (2) Polaire : le culte de **Myōken** (étoile polaire) fournit le *mikazuki* des Chiba ; le mon en est un dérivé d'emblème de culte local, à trois degrés de la doctrine — non instruit. (3) Solaire : le *kiku* à seize pétales est une rosace de plein droit, mais le seize y est un compte de pétales, pas un nombre exposé — **l'erreur de catégorie que la fiche Gizeh signale est ici exactement celle qui menaçait**, écartée. (4) Métrologique 28 : **recherché, absent** ; les comptes attestés sont 1 à 10 et 16.
- **Argument porté avec sa réfutation.** La prédominance massive du ternaire dans le corpus se prêtait à une lecture symbolique. Elle est **refusée par une objection technique décisive** : le ternaire est le groupement le moins coûteux à tracer juste au compas (report du rayon, un point sur deux), ce qui suffit à en rendre compte. L'argument favorable n° 3 du discernement est donc porté **avec sa réfutation**, et signalé comme le plus faible des trois.
- **Discernement ouvert, verdict réservé (Cmd 12).** Ni l'universalité du symbolisme ni son inégale conservation ne sont en cause — ce sont des doctrines reçues au dépôt. Seule l'**application au corpus des kamon** est instruite. **Distinction proposée** : support formel (réel, contraignant, transmis par corps de métier — relève de la question du métier traditionnel, donc de la fiche sashimono jumelle) / science du sens (**non attestée** — ce qui en tient lieu se ramène au rébus phonétique, à la vertu naturelle et à la fonction) / symboles transportés (*manji*, roue, joyau, *kuyō*, *rokumonsen* — réels, mais **empruntés au bouddhisme** ; le mon les transporte, il ne les a pas produits). La proposition est **réfutable et la réfutation est nommée** : que soit produit un corps d'interprétation japonais du mon. Le joint kamon ↔ métier traditionnel **n'est pas posé** : il attend le verdict de la fiche sashimono.
- **Vérification** — `verifier-invariants.py` : **1415 fichiers, 0 erreur, 0 avertissement** (1411 avant la passe : les quatre fiches nouvelles sont bien dans le périmètre). Cmd 15 : 8 fichiers contrôlés, 0 caractère invisible.
- **Épreuve du contrôle (§VII) — le refus a été observé.** `verifier-invariants.py` n'a pas été modifié cette session, mais il n'avait jamais été **vu refuser** sur ces fiches. Quatre fautes ont donc été fabriquées dans une **copie jetable** du dépôt (jamais dans le dépôt vivant) : `sources_count: 4` sur *tomoe* → **B2**, lien mort dans le glossaire → **C1**, wikilink `doctrinal/` → `label/` dans le discernement → **C3**, `updated:` supprimé sur l'établi → **B1**. Résultat : **vert sur l'état sain (0 erreur), refus sur les quatre fautes (3 erreurs + 1 avertissement), chacune nommée sur le bon fichier.** Le contrôle mord.
- **Graphe** — régénéré (`generer-cartographie.py`, 567 nœuds, 1930 arêtes). Les trois fiches doctrinales sont **correctement maillées** (10 arêtes). **Une anomalie, signalée et non corrigée d'office** : `label/direction-artistique/amorcage/kamon-personnel.md` remonte « isolée ». La cause n'est pas un défaut de la fiche mais une **limite du Sceau label** — il offre `liens` (internes au circuit) et `liens_atelier`, mais **aucun champ pour le renvoi `label/` → `doctrinal/` que `label/CLAUDE.md` autorise pourtant explicitement**. Toute fiche label dont les seuls renvois sortants sont doctrinaux apparaîtra donc isolée. Vérifié : aucune fiche `label/` du dépôt ne déclare de cible doctrinale en cartouche — le renvoi en corps de texte est la pratique établie. **Verdict à Sidy** : ajouter un champ (`liens_doctrinal`, sur le modèle de `liens_atelier`), ou tenir l'avertissement pour un faux positif connu. La machine ne tranche pas.
- **Constat annexe** — le `graphe-cartographie.json` versionné était **périmé de huit fiches** produites par d'autres sessions (INF-16, OUT-08, Futūḥāt, Shayegan, convention `textes/`) : sur les 12 nœuds apparus à la régénération, **4 seulement** relèvent de cette passe. Signalé sans correction : la régénération de ce commit remet le manifeste à jour.
- **Commit** : 8d4159c
...
- **Fiche créée** : `doctrinal/symboles/golem.md`
- **Type** : symbole, statut traditionnel, tradition_cadre judaïsme
...
- [[atelier/rd/outillage/2026-08-10_methode-traitement-lot-discernement-sashimono]]
  ...
  (2026-06-20) ».
- (cf. Domaine Réservé, fiche `sidy`) — ajout d'une section « Repères chronologiques
...
- **Anomalies** : Sceau Recteur incomplet (`tradition_cadre`, `sources`,
  ...
  `(cf. Domaine Réservé, fiche `philosophie-sashimono`)` et `[[atelier/rd/index.md]]` violaient
...
- **Fiche** : `2026-08-11_trois-territoires-inacheve-flux-speculatif-exploratoire.md`
  - **Contenu** : architecture de flux reliant Discernement (status: speculatif),
    R&D (statut_experience: exploratoire), et Herméneutique registre:expression —
    nature commune (non-finalité, portance *zōsaku*, validation humanelle requise),
    flux spéculatif→exploratoire→théorie adoptée/archivage, légitimité théurgique
    (Laboratoire comme Athanor de Transmutation), distinction Doctrine (transmission
    du Principe, descendante) vs. Théorie (articulation phénoménale, ascendante,
    contingente).
  ...
  - **Statut** : `status: adopte` (verdict 2026-08-11). Fiche reste en Discernement
    ...
    pas transmission du Principe).
  - **Enrichissements** : 3 commits tracés — création initiale, enrichissement
    théurgique/alchimique, nuance Doctrine/Théorie.
- **Validation** : `verifier-invariants.py --racine /root/wiki` : 5 erreur(s)
  pré-existantes, 40 avertissement(s) baseline (inchangé).
...
- **Enrichi** : [[doctrinal/symboles/malamatiyya]] (juridiction des Afrâd, guidance d'al-Khiḍr, `to-source`) ; [[doctrinal/sources/guenon-regne-quantite-ch28-40-contre-initiation]] (ch. XXXIX note 7, double face de Metatron) ; [[doctrinal/deviations/reincarnation-vies-anterieures]] (appui primaire HDV ch. XXII) ; [[doctrinal/symboles/wahdat-al-wujud]] (irréciprocité Brahma/Monde, HDV ch. XXVI) ; [[doctrinal/index.md]].
...
- **Créé** (fiche discernement) : 🔍 [[doctrinal/discernement/2026-07-27_septenaire-transversal-balance-degre-soleil]] — statut `en cours`, plan consigné en (cf. Domaine Réservé, fiche `plan-fiche-discernement-septenaire-transversal-2026-07-27`), rédaction déléguée session suivante
...
- **Enrichi** : [[doctrinal/index.md]] (§VI sources + §VII discernement), (cf. Domaine Réservé, fiche `bibliotheque-physique`) (section III).
...
- **Corrigé** : (cf. Domaine Réservé, fiche `bibliotheque-physique`) — remplacement « Albin Michel, choix de textes » par édition Penot/Entrelacs 2009 confirmée (ISBN 978-2-908606-60-7).
...
- **Addendum** : (cf. Domaine Réservé, fiche `bibliotheque-physique`) enrichi (section VI, 9 ouvrages — texte de référence + fondations + garde-fou + sources primaires).
...
- **Signalement backlog** : ce qui reste à transcrire pour compléter le récit eschatologique (texte discursif du ch. 198 ; intro/annotations Gloton ; renvoi *Jésus fils de Marie* pp. 466-470 ; parallèle *ʿUqlat al-mustawfiz* Nyberg pp. 39-99) est consigné en deux endroits — section « À transcrire » de la fiche source et §E du backlog (cf. Domaine Réservé, fiche `04-sessions-par-fonction-et-backlogs`). Item « déposer dans raw/ » du §E marqué FAIT.
...
- **meta/ (outillage)** : classé (cf. Domaine Réservé, fiche `09-briefing-transition-qwen36-27b-2026-07-01`) (briefing transition modèle local Ornith-1.0-9B → **Qwen3.6-27B-FP8** : bilan tests Ornith, justification, 3 changements du runbook §3.2, état Pod RunPod, actions A→H). Nom normalisé en minuscules (§III). Référencé dans (cf. Domaine Réservé, fiche `README`) (Outils opérationnels). Confiné à `meta/` — aucun croisement d'étanchéité.
...
- **Actualisé au 2026-07-01** l'ensemble du contexte lu par la session Claude.ai (iPad) après le grand ingest :
  (cf. Domaine Réservé, fiche `briefing-claude-ai`) (§11 état + §12 inventaire), (cf. Domaine Réservé, fiche `01-contexte-demarche-etat`) (snapshot),
  (cf. Domaine Réservé, fiche `02-instrument-feuille-de-route`) (v0.2, convergence des 28 établie, spec axe 38°, question §8.2),
  (cf. Domaine Réservé, fiche `04-sessions-par-fonction-et-backlogs`) (backlogs questions/vigilance/discernement + nouveau §E ingests à venir ; 14→15 discernements), (cf. Domaine Réservé, fiche `README`).
...
- **meta/** : (cf. Domaine Réservé, fiche `bibliotheque-physique`) enrichie (Gaston Georgel, Platon *Œuvres complètes* Flammarion/Brisson, Burckhardt *Clés spirituelles de l'astrologie musulmane*, Meftah Albouraq, Ibn ʿArabī *De la mort à la résurrection*).
...
- **VIGILANCE** :
  1. **Étanchéité §VI corrigée (signalée, non silencieuse — Cmd 7)** : la fiche-source `archeometre-saint-yves-papus-1911` mentionnait « bibliothèque de Sidy » + un lien `(cf. Domaine Réservé, fiche `bibliotheque-physique`)` (fait personnel + lien neutre→sensible, interdit) → **neutralisé** ; l'appartenance/édition est enregistrée côté (cf. Domaine Réservé, fiche `bibliotheque-physique`) (sens autorisé meta→doctrinal).
...
- **Rangé** : (cf. Domaine Réservé, fiche résultats du test doctrinal).
...
- **Rangé** : (cf. Domaine Réservé, fiche résultats finaux) (via `_inbox/`, frontmatter meta conforme) ; lié à (cf. Domaine Réservé, fiche 06).
...
- **Mises à jour** : runbook (cf. Domaine Réservé, fiche 05), (cf. Domaine Réservé, fiche 03) et (cf. Domaine Réservé, fiche 04) complétés. Rien de la session de test n'a touché le vrai dépôt (bac à sable isolé). Lot *archéomètre* du sas toujours non traité.
...
- **Rangé** : (cf. Domaine Réservé, fiche compte-rendu du 1er test) (via `_inbox/`, frontmatter meta conforme — aucune correction).
...
- **Correctifs intégrés** au runbook (cf. Domaine Réservé, fiche 05) et à (cf. Domaine Réservé, fiche 03)/(cf. Domaine Réservé, fiche 04) : RunPod **Pod** (pas Serverless), authentification via `ANTHROPIC_CUSTOM_HEADERS` (vLLM exige `Authorization: Bearer`), `--max-model-len 131072` (plancher réaliste), pièges sshd/port/`pkill`.
...
- **Source** : (cf. Domaine Réservé, fiche Bibliothèque physique de travail) (recension validée par Sidy, 2026-06-28), pages existantes du wiki, et quelques candidats hors-possession signalés pour `raw/`.
...
- **Modifié** : [[doctrinal/discernement/2026-06-20_triptyque-medine-jeu-de-piste|triptyque-medine]] (#100, deux recoupements majeurs : « jeu de piste » comme méthode généralisée #102 + 3 objets #99/#101/#104), [[atelier/projets/album-personnel|album-personnel]] (enrichi : label/vinyle #78, UAD 6176 #136, perception auditive #42, identité multi-supports #110), (cf. Domaine Réservé, fiche bourdonnement-tempe) (3e occurrence #77).
...
- **Créé** : [[atelier/projets/album-personnel|album-personnel]], 3 fiches [[atelier/materiel/neve-1073spx|matériel]] audio (neve-1073spx, tascam-model-12, technics-su-8080), (cf. Domaine Réservé, fiche taekwondo-hansu) (#105, classé meta/ et non atelier — ni audio ni création artistique au sens du protocole).
...
- **Opération** : ARCHIVAGE & MAILLAGE
- **Créé** : profil (cf. Domaine Réservé, fiche `sidy`),  ; stubs d'autorités/formes : [[doctrinal/autorites/ibn-arabi]], [[doctrinal/autorites/al-ghazali]], [[doctrinal/autorites/rene-guenon]], [[doctrinal/autorites/platon]], [[doctrinal/traditions/sanatana-dharma]], [[doctrinal/traditions/ahl-al-sunnah-wa-l-jamaa]], [[doctrinal/traditions/tasawwuf]]
