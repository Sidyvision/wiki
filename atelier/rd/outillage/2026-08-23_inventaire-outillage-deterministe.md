...
| `atelier/rd/outillage/transcrire-audio-whisper.py` | Transcription **locale** d'un audio (faster-whisper, venv `.whisper-venv/`, non versionné) en Markdown horodaté ; **tranches** ffmpeg pour borner la mémoire (3 Go) ; fichier `.partiel` renommé seulement à la fin ; refus d'écraser (ajouté 2026-09-15) | pour toute source orale (vidéo, conférence) dont l'audio est dans `raw/` |
| `atelier/rd/outillage/convertir-jabre-munqidh-arabe.py` | OCR `tesseract -l ara` (300 dpi, `--psm 6`) d'un texte arabe relié à l'arabe : sortie en **ordre de lecture**, double numérotation PDF / imprimée, retrait compté des seuls invisibles Cmd 15 injectés par le moteur ; cache des sorties brutes ; refus d'écraser (ajouté 2026-09-15) | modèle pour un imprimé arabe net, sans le prétraitement ×2 + Otsu d'OUT-08 |
