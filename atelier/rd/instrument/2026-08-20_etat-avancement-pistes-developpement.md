...
1. ~~Feuille de route périmée~~ — **corrigé, constat périmé (2026-08-25)** : `instrument-feuille-de-route-v2.md` (`updated: 2026-08-25`) désigne désormais, dans son bandeau d'ouverture, l'architecture **v0.3** comme document canonique. Correction déjà effectuée dans une session antérieure à la présente relecture, non répercutée ici jusqu'à ce jour — voir §3 P5.11 ci-dessous.
...
4. ~~Soumissions « Gem » jamais closes formellement~~ — **corrigé, constat périmé (2026-08-25)** : les 4 fiches `soumission-gem-*.md` portent désormais chacune un bandeau « **Absorbée (2026-08-25)** » avec pointeur exact vers le document qui a repris leur contenu (`spec-technique-axe-38-degres.md` et/ou architecture v0.3 §8), et la précision qu'elles ne constituent pas un point d'action ouvert (Cmd 10 : trace conservée, non un point actif). Correction déjà effectuée dans une session antérieure à la présente relecture, non répercutée ici jusqu'à ce jour — voir §3 P5.12 ci-dessous.
...
8. ~~Statuer sur le paramètre d'époque et l'origine du zodiaque (tropical/sidéral)~~ — **tranché le 2026-08-25** (commits `6deaf2b`/`b2acd1b`, session antérieure de moins d'une heure à la présente passe — correction du même jour d'un constat erroné de ma part). Verdict : les deux modes sont implémentés et commutables via `zodiaque.mode_zodiaque: tropical | sideral` (`instrument-donnees.yaml` v0.5.2), `spec-anneau-zodiacal.md` §3.3 mis à jour en conséquence, §7 point 3 clos. **Reste ouvert, sous-point distinct** : le choix de l'école d'ayanamsha en cas de mode sidéral (`zodiaque.ayanamsha_deg: null` — Lahiri, Fagan-Bradley, ou valeur de source traditionnelle) — non tranché, non bloquant tant que `mode_zodiaque: tropical` reste la valeur active.
9. Décider si l'hypothèse H3 de `2026-07-26_investigation-referentiels-stellaires-cycles.md` (convergence Gizeh/Idrīs/Hermès au degré 24) — qualifiée par le document lui-même de « peut-être la pièce la plus neuve » mais non instruite, `status: speculatif`, `to-source` — mérite l'ouverture d'une fiche `discernement` dédiée avant toute intégration. **Tranché le 2026-08-25** (Sidy : « Tu peux, oui et à mon sens l'hypothèse est très plausible ») : fiche ouverte,
   [[doctrinal/discernement/2026-08-25_gizeh-degre-24-solaire-hermes-idris]], `Statut : en cours` — les trois maillons de la chaîne (Gizeh=Tombeau d'Hermès/Idrīs ; Idrīs=degré 24/Ciel du Soleil, déjà clos ailleurs) sont sourcés en doctrinal/, mais la synthèse propre (ancrer Gizeh *lui-même* au degré 24) est nouvelle et n'a pas reçu de verdict de clôture formel — l'appréciation de plausibilité de Sidy est consignée comme telle, distincte d'une clôture sur texte primaire (Cmd 12).
10. ~~Constituer la bibliothèque prioritaire~~ — **correction du 2026-08-25** : ce constat était périmé. Recoupement avec `atelier/rd/bibliotheque/catalogue-bibliotheque.md` (§I) : sur les 6 ouvrages de priorité 1-3 listés au §7 de l'investigation, **5 sont déjà en bibliothèque physique confirmée** — *Le Roi du Monde*, *Formes traditionnelles et Cycles cosmiques*, *Symboles de la Science sacrée*, *La Grande Triade* (tous §I) et Tilak *Origine polaire de la tradition védique* = trad. française de *The Arctic Home in the Vedas* (déjà indexé, `index-origine-polaire-tilak.md`, intégré 2026-08-25). Seul manque un texte distinct du même auteur : ***The Orion* (1893)**, premier livre de Tilak — nuance signalée par l'investigation elle-même (§1.4), non substituable par l'*Arctic Home* déjà possédé. Restent hors bibliothèque physique, priorité 3-4, non traditionnels (académiques) : Neugebauer & Parker, Textes des Pyramides, Kunitzsch/Ibn Qutayba — non prioritaires pour une instruction de matière traditionnelle. **Conclusion** : la bibliothèque nécessaire à l'instruction de H1/H2/H3 est essentiellement déjà réunie ; seule acquisition potentielle restante identifiée : Tilak, *The Orion*.

**P5 — hygiène documentaire (bas risque, exécutable rapidement sur validation)**
11. ~~Remettre à jour le bandeau de `instrument-feuille-de-route-v2.md` pour pointer vers la v0.3~~ (écart §2.1) — **fait, constaté le 2026-08-25** (§2.1).
12. ~~Marquer les 4 fiches `soumission-gem-*` comme absorbées, avec pointeur vers les documents qui en ont repris le fond~~ (écart §2.4) — **fait, constaté le 2026-08-25** (§2.4).
