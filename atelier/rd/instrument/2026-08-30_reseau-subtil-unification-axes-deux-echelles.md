---
title: "Le réseau subtil : unification des axes et résolution par les deux échelles"
type: projet
statut_experience: exploratoire
tags: [instrument, axe-unifie, nadis, reseau, macrocosme, microcosme, lataif, aqtab, registres, deduction]
created: 2026-08-30
updated: 2026-08-30
sources: []
links: ["[[doctrinal/discernement/2026-08-29_axe-du-monde-doctrine-transversale]]", "[[doctrinal/discernement/2026-07-26_adam-qadmon-insan-kamil-wang-vaishvanara]]", "[[doctrinal/discernement/2026-07-16_sept-poles-aqtab-malakut-planetaire]]", "[[doctrinal/discernement/2026-08-29_sept-poles-sept-lataif]]", "[[doctrinal/discernement/2026-08-29_sephiroth-chakras-sept-niveaux]]", "[[doctrinal/symboles/polarite-laterale-de-l-axe]]", "[[doctrinal/sources/guenon-kundalini-yoga-etudes-hindouisme]]", "[[doctrinal/sources/avalon-serpent-power-nadis-reseau]]", "[[doctrinal/sources/guenon-homme-devenir-vedanta-ch21-artere-coronale-rayon-solaire]]", "[[atelier/rd/instrument/spec-anneau-zodiacal]]"]
---

# Le réseau subtil : unification des axes et résolution par les deux échelles

> Circuit **atelier**, pôle R&D. Document d'instrumentation et de déduction, non
> doctrinal. Liens vers `doctrinal/` en sens unique, signalés.
>
> **Nature** : travail de déduction conceptuelle demandé par Sidy le 2026-08-30,
> à partir de deux de ses énoncés — l'unification des axes verticaux, et la
> non-exclusivité de la résidence prophétique aux cieux planétaires et de
> l'attribution aux *laṭāʾif*. Ce document **déduit** ; il ne tranche pas
> (Cmd 12). Ce qu'il propose de nouveau est signalé comme tel.

## 1. Le point de départ : un seul Être

L'énoncé de Sidy, reformulé fidèlement : *l'axe du monde, le filament de
l'Insān al-Kāmil, le Pôle, la colonne du milieu sont la même chose* — et *il ne
s'agit que d'un seul Être, d'où la correspondance macrocosme-microcosme*.

Le dépôt porte déjà, séparément, chacun des maillons :

| Terme | Où il est établi |
|---|---|
| *skambha* = *qutb* = *stauros* | [[doctrinal/discernement/2026-08-29_axe-du-monde-doctrine-transversale]] (validé) |
| + *Mêru* / *Mêru-danda* | même fiche, complétée le 2026-08-29 |
| Adam Qadmôn = al-Insān al-Kāmil = Wang = Vaishwânara | [[doctrinal/discernement/2026-07-26_adam-qadmon-insan-kamil-wang-vaishvanara]] (clos) |
| colonne du milieu = *suṣumnā* | [[doctrinal/symboles/polarite-laterale-de-l-axe]] (Guénon, texte) |
| les sept Pôles siègent aux degrés 21-27 | [[doctrinal/discernement/2026-07-16_sept-poles-aqtab-malakut-planetaire]] (clos) |

**Ce qui manquait n'était pas un maillon, c'était la fermeture de la chaîne.**
Chaque identité était établie de son côté ; aucune n'était rapportée aux autres
comme désignant *un même axe unique*. C'est l'objet du verdict du 2026-08-30.

## 2. La résolution de la contradiction apparente

### 2.1 Ce que j'avais posé, et pourquoi c'était faux

Le 2026-08-29, j'ai bloqué le rapprochement sept Pôles ↔ sept *laṭāʾif* sur le
constat suivant : les deux séries prophétiques ne coïncident ni en composition
ni en rang (ordre du *miʿrāj* d'un côté, ordre de Semnânî de l'autre), donc
« aucun appariement terme à terme n'est écrivable ».

**Le constat était exact ; l'objection ne l'était pas.** Elle supposait qu'une
bijection devait exister entre les deux séries. Rien ne l'exigeait. J'ai comparé
deux listes qui ne sont pas deux *ordres* d'une même relation, mais **deux
relations distinctes** des mêmes Prophètes au même axe :

| Relation | Nature | Série |
|---|---|---|
| **Résidence** | où le Prophète **siège** dans la hiérarchie cosmique | les sept cieux planétaires (*miʿrāj*) |
| **Régence** | quel centre subtil le Prophète **gouverne** dans l'être humain | les sept *laṭāʾif* (Semnânî) |

Un même Prophète peut résider au ciel de Saturne et régir un centre subtil sans
la moindre contradiction : ce ne sont pas deux places, ce sont **une place et
une fonction**. Comme le formule Sidy : les deux ne sont pas mutuellement
exclusifs.

### 2.2 Pourquoi les ordres diffèrent — un principe déjà validé au dépôt

Reste à expliquer que les rangs ne coïncident pas. La réponse n'exige aucune
doctrine nouvelle : **le dépôt a déjà tranché ce cas de figure**, pour l'anneau
zodiacal. Douze signes et vingt-huit *manāzil* partitionnent le même anneau,
leurs frontières ne coïncident jamais, et
[[atelier/rd/instrument/spec-anneau-zodiacal]] pose que *« le décalage est une
donnée, pas un défaut de rendu »*.

Deux partitions non commensurables d'un même continu ont des bornes qui ne se
correspondent pas. Les sept cieux et les sept *laṭāʾif* sont deux partitions
non commensurables du même axe. **Le décalage des Prophètes est une donnée du
même ordre.** Art. 3 du sashimono : jamais de joint forcé — et ici, aucun joint
n'a besoin d'être forcé, parce qu'aucun n'était requis.

### 2.3 La comparaison correcte : échelle contre échelle

C'est la conséquence qui a le plus de portée. Si résidence et régence sont deux
relations à deux **échelles** — cosmique et humaine — alors comparer les cieux
planétaires aux *laṭāʾif* revient à comparer un macrocosme à un microcosme.
**C'est une erreur de catégorie, et c'est celle que j'ai commise.**

Les comparaisons tractables sont celles qui respectent l'échelle :

```
        MACROCOSME                          MICROCOSME
   ┌──────────────────────┐           ┌──────────────────────┐
   │ 7 cieux planétaires  │  ←──?──→  │ 7 laṭāʾif            │   ✗ erreur d'échelle
   │   (Islam)            │           │   (Islam)            │
   └──────────────────────┘           └──────────────────────┘
            ↕ ?                                 ↕ ?
   ┌──────────────────────┐           ┌──────────────────────┐
   │ 7 lokas              │           │ 7 chakras            │
   │   (Inde)             │           │   (Inde)             │
   └──────────────────────┘           └──────────────────────┘

   Comparaisons LICITES :  cieux ↔ lokas  (macro ↔ macro)
                           laṭāʾif ↔ chakras  (micro ↔ micro)
   Correspondance INTERNE à chaque tradition : macro ↕ micro
```

### 2.4 Vérification : chaque tradition porte-t-elle bien les deux échelles ?

La déduction serait creuse si les deux échelles n'étaient pas attestées de
chaque côté. Elles le sont, et par les sources déjà au dépôt :

- **Inde** — Guénon donne les deux séries **et leur appariement**, dans le texte
  archivé le 2026-08-29 : à chacun des six *chakras* correspond une déité qui a
  « d'autre part, au point de vue "macrocosmique", sa demeure » dans un *loka*
  (*Bhûrloka*, *Bhuvarloka*, *Swarloka*, *Janaloka*, *Tapoloka*, *Maharloka*,
  et *Satyaloka* pour *sahasrāra*). Il conclut : « **tous ces mondes ont leur
  correspondance dans les "centres de conscience" de l'être humain** ».
  ➜ macro et micro appariés, à l'intérieur d'une même tradition, par l'auteur.
- **Islam** — le ch. IV du commentaire de Shayegan, transcrit et relu le
  2026-08-29, atteste que « chez Najm Kobrâ, **les sept cieux astronomiques sont
  homologués avec les sept cieux intérieurs de l'âme** », et que les sept cieux
  « apparaissent aussi comme les sept prophètes de "ton être" dans la doctrine
  de Semnânî ».
  ➜ macro et micro appariés, à l'intérieur de la même tradition, par les
  auteurs eux-mêmes. **La pièce que je cherchais était déjà là.**
- **Kabbale** — Guénon pose la prémisse de sa propre comparaison : les
  *Sephiroth* « doivent nécessairement avoir, **comme toutes choses, leur
  correspondance dans l'être humain** ».
  ➜ les Sephiroth sont des émanations (macro) *dont il prend la correspondance
  humaine* (micro).

### 2.5 Conséquence rétroactive sur le verdict Sephiroth ↔ chakras

Elle mérite d'être relevée, car elle **conforte** un verdict déjà rendu sans
rien y changer. Guénon ne compare pas les Sephiroth *en tant qu'émanations
cosmiques* aux *chakras* : il annonce d'abord qu'il prend les Sephiroth dans
leur correspondance humaine, puis compare. Sa comparaison est donc **micro ↔
micro** — exactement la forme licite selon §2.3. Le verdict du 2026-08-29 était
juste, et l'on voit maintenant *pourquoi* il était méthodologiquement bien
formé. Le détail des localisations corporelles qu'il donne (bras et épaules pour
*Hesed*/*Geburah*, hanches pour *Netsah*/*Hod*) le confirme : il travaille dans
le corps, donc au microcosme.

## 3. Ce que le réseau des nâdîs apporte

### 3.1 Un réseau, pas trois lignes

Les *nâdīs* sont « les lignes de direction que suivent les forces vitales ».
Trois sont principales, mais Guénon précise que le *mūlādhāra* est « la racine
(*mūla*) de *suṣumnā* **et de toutes les *nāḍīs*** ». L'axe n'est donc pas une
tige isolée : c'est le **tronc d'un réseau connexe**, enraciné en un point.

D'où une articulation en trois termes qu'il faut tenir distincts :

| Élément du réseau | Rôle | Ce qu'il n'est pas |
|---|---|---|
| **l'axe** (*suṣumnā*) | le tronc, porte les centres | pas un degré parmi d'autres : la coordonnée elle-même |
| **les centres** (*chakra*, *laṭāʾif*, Sephiroth…) | les **nœuds** où le réseau se concentre | pas des lieux séparés : des concentrations du même réseau |
| **les canaux latéraux** (*iḍā*, *piṅgalā*) | la **polarité de circulation** | ne portent aucun centre — jamais une étape |

### 3.2 La déduction centrale

Si les centres sont les nœuds d'**un seul** réseau, et si les Prophètes en sont
les régents, alors les « sept prophètes de ton être » ne sont pas sept figures
juxtaposées : ce sont **sept fonctions d'une seule structure intérieure**. Et
comme il n'y a qu'un seul Être, ces mêmes fonctions, lues à l'échelle cosmique,
sont les résidences des mêmes Prophètes aux sept cieux.

**Résidence et régence sont le même fait, lu à deux échelles.** C'est pourquoi
ce sont les mêmes noms des deux côtés — et c'est aussi pourquoi les rangs ne se
recouvrent pas : les deux lectures partitionnent différemment.

Le principe est celui que Guénon cite du *Vishwasāra Tantra*, et qui gouverne
tout ce document : « **Ce qui est ici est là, ce qui n'est pas ici n'est nulle
part** » (*Yad ihāsti tad anyatra, yan nēhāsti na tat kwachit*).

### 3.3 Une ramification qui reste à instruire

Si l'axe est le tronc d'un réseau, les *nāḍīs* secondaires devraient avoir leur
répondant dans les autres traditions. Guénon en donne l'amorce en note : « les
différents "canaux" qui relient les Séphiroth entre elles ne sont pas sans
analogie avec les *nāḍīs* ». **Les vingt-deux sentiers de l'Arbre séphirothique
sont des canaux reliant des nœuds** — même forme que le réseau des *nāḍīs*.

C'est une piste sérieuse et **non instruite** : elle demanderait d'examiner si
les sentiers se laissent lire comme un réseau de circulation, et non seulement
comme une table de relations. Signalée, non tranchée. Elle rejoint une suite
déjà proposée le 2026-08-20 (« enrichir le registre `qabbalah` : 22 sentiers »).

> **Mise à jour du 2026-08-30 — la piste est devenue un chantier ouvert.** Sidy
> a formulé l'hypothèse en propre : les *nâdîs* issues du cœur et les sentiers
> séphirothiques seraient **identiques**, le réseau rayonnant de **Tiferet**.
> Instruite en fiche de discernement :
> [[doctrinal/discernement/2026-08-30_nadis-du-coeur-sentiers-sephirothiques-tiferet]]
> (`speculatif`, **verdict réservé**, lien `rd/` → `doctrinal/` en sens unique).
>
> **Conséquence pour l'Instrument, à connaître avant d'y toucher** : le chantier
> est **bloqué en amont**, et pas pour une raison doctrinale. La fiche
> [[doctrinal/sources/kabbale-10-sefirot-structure]] déclare elle-même que les 22
> sentiers ne sont **pas documentés** ; et la tradition connaît **plusieurs
> arrangements incompatibles** du tracé des voies, dont le dépôt n'a fixé aucun.
> Le nombre de sentiers aboutissant à Tiferet dépend donc d'un choix non fait :
> toute convergence numérique qu'on croirait constater serait **fabriquée par ce
> choix implicite**. Le premier acte est de **fixer l'arrangement sur source**.
>
> Même verdicté, ce joint ne serait **pas** un ancrage inter-registres : il ne
> joint aucun niveau de l'axe. Il relèverait du régime du joint Janus —
> équivalence entre **nœuds notionnels** (§8.5 et fiche Janus §5).

## 4. Ce que l'unification implique pour l'Instrument

### 4.1 L'état actuel : un axe dessiné plusieurs fois

Le prototype trace aujourd'hui, en parallèle, plusieurs verticales qui sont —
selon le verdict — **une seule** :

| Objet rendu | Position actuelle |
|---|---|
| « axe neutre du Principe » | x = 0, z = 0 |
| filament al-Insān al-Kāmil | x = 0,6 |
| nœud Homme Universel | x = −1,7 |
| colonne du milieu de chaque registre | x = 0 mais z = −6,0 − 3,2·n |

Ce sont quatre expressions d'un même axe, séparées **pour la seule lisibilité**.
Le décalage y est un artefact de rendu — à la différence du décalage
signes/*manāzil*, qui est, lui, une donnée.

> **Distinction à ne pas manquer** : il y a désormais **deux sortes de décalage**
> dans l'Instrument, qu'il serait grave de confondre.
> — *Décalage-donnée* : deux partitions non commensurables du même continu (12
>   signes / 28 *manāzil* ; cieux / *laṭāʾif*). **Ne doit jamais être ajusté.**
> — *Décalage-artefact* : le même objet dessiné à plusieurs endroits pour rester
>   lisible (les axes ci-dessus). **Doit pouvoir être résorbé.**

### 4.2 Proposition : un mode « axe unifié », et un champ d'échelle

Deux gestes, l'un de rendu, l'autre de donnée :

1. **Mode de rendu commutable.** Une vue « éclatée » (l'actuelle, lisible) et
   une vue « **axe unifié** » où les colonnes centrales de tous les registres et
   le filament se rabattent sur l'axe unique — donnant à voir que les partitions
   sont celles d'une seule verticale. Le décalage-artefact devient alors
   explicitement ce qu'il est : un réglage d'affichage.
2. **Champ `echelle` sur les registres.** `macrocosmique | microcosmique |
   transcalaire`, pour que la règle du §2.3 soit portée par la donnée et non par
   la seule prose — et, à terme, contrôlable mécaniquement (un ancrage entre
   deux registres d'échelles déclarées différentes mériterait au minimum un
   avertissement).

Attribution proposée, **à soumettre** — c'est une lecture, pas un constat :

| Registre | Échelle proposée | Motif |
|---|---|---|
| `tasawwuf` (Ḥaḍarāt / 38 degrés) | `transcalaire` | degrés ontologiques, ni proprement cosmiques ni proprement humains |
| `qabbalah` (Sephiroth) | `transcalaire` | émanations cosmiques **dont Guénon prend la correspondance humaine** pour comparer |
| `hindouisme-tantra` (chakras) | `microcosmique` | centres « localisés » dans le corps humain, explicitement |
| `vedanta` (états d'Âtmâ) | `transcalaire` | Vaishwânara est à la fois *Virāj* (macro) et l'état de veille individuel (micro) |

Le fait que trois registres sur quatre soient *transcalaires* n'est pas un
échec de la classification : c'est cohérent avec le principe même
(macrocosme/microcosme comme deux lectures d'un unique Être). Le champ sert
surtout à **isoler les registres franchement mono-échelle**, qui sont ceux où
une comparaison directe risque l'erreur de catégorie.

## 5. Ce qui reste hors de portée, et pourquoi

Honnêteté de méthode — la déduction ci-dessus **ne produit aucun appariement
terme à terme** :

- Elle explique *pourquoi* les deux séries prophétiques diffèrent, et lève
  l'objection de forme. Elle ne fournit pas la liste ordonnée des sept *laṭāʾif*
  et de leurs régents, qui manque toujours au dépôt.
- La pièce identifiée reste la même : le **ch. II du commentaire de Shayegan**
  (« Les disciplines spirituelles », §III), qui porte l'exposé des sept centres
  de Semnânî et leur homologie aux sept *cakra*. Ouvrage en bibliothèque
  physique, chapitre non transcrit.
- Avec ce chapitre, la comparaison *laṭāʾif* ↔ *chakra* (micro ↔ micro) devient
  directement instruisible — et c'est celle qui a du sens, non celle que
  j'avais tentée.

## 6. Suites proposées

1. **Transcrire le ch. II de Shayegan** — seule pièce manquante, et elle
   débloque la comparaison licite.
2. **Implémenter le mode « axe unifié »** et le champ `echelle` (§4.2).
3. **Instruire les cieux planétaires ↔ *lokas*** (macro ↔ macro) : les deux
   séries sont au dépôt, la comparaison est licite, elle n'a jamais été tentée.
4. ~~Piste de fond, non prioritaire~~ → **chantier ouvert le 2026-08-30** : les
   22 sentiers séphirothiques lus comme réseau de canaux rayonnant de Tiferet
   (§3.3). **Bloqué en amont** : l'arrangement des sentiers n'est pas fixé au
   dépôt.
5. **Trancher la bifurcation des centres surnuméraires** (§7.3) — trois options
   posées, aucune implémentée.
6. **Statuer sur les divergences table/planches** (§7.5) — deux cellules vides
   d'*Ājñā*, à combler ou à laisser.
7. **Trancher la figuration de la discontinuité** (§8.6) — trois pistes posées,
   aucune implémentée ; c'est la réserve doctrinale la plus lourde que le rendu
   actuel ne porte pas.
8. **Piste ouverte, non instruite** : le parallèle islamique sur la couronne de
   la tête, signalé par Guénon lui-même (ch. XXI, note 3) sans texte nommé.

## 7. Acquis des planches et des centres surnuméraires (2026-08-30, second relevé)

Le relevé des huit planches d'Avalon et du chapitre V complet
([[doctrinal/sources/avalon-serpent-power-nadis-reseau]] — lien `rd/` →
`doctrinal/`, sens unique, signalé) apporte un fait qui touche non pas le
contenu d'un registre, mais **le statut même de la notion de registre**.

### 7.1 Le fait

Avalon écrit que les six *cakra* du *Shatchakranirûpana* « sont **les
principaux** » et qu'« il existe une **très longue liste** de Chakra ». Il en
nomme cinq autres, et — c'est le point — **situés dans les intervalles de la
série reçue**, non au-delà d'elle : un lotus à 8 pétales *au-dessous*
d'*Anāhata* ; *Lalanā* (12) *au-dessus* de *Vishuddha* et légèrement
*au-dessous* d'*Ājñā* ; *Manas* (6) et *Soma* (16) *au-dessus* d'*Ājñā* et
*au-dessous* de *Sahasrāra* ; un lotus blanc à 12 pétales *dans le péricarpe*
de *Sahasrāra*.

### 7.2 Ce que cela confirme du modèle

C'est une **confirmation indépendante** du choix d'architecture déjà fait au
dépôt, et elle vient de l'intérieur d'une seule tradition, ce qui la rend plus
forte qu'un argument comparatif :

> Un registre n'est pas *l'axe* ; c'est **une partition de l'axe, faite par un
> texte donné, pour un usage donné**.

La preuve est ici administrée sans sortir de l'hindouisme : deux textes de la
même tradition partitionnent le même axe en six et en onze, et le second ne
contredit pas le premier — il le raffine. Si l'écart entre deux partitions
**intra**-traditionnelles n'est pas une contradiction, alors l'écart entre deux
partitions **inter**-traditionnelles ne l'est pas davantage. Le *décalage-donnée*
est confirmé comme régime normal, et non comme anomalie à réduire.

### 7.3 Ce que cela change pour l'Instrument — et ce que cela ne change pas

**Cela ne change rien aux données actuelles.** Le registre `hindouisme` reste à
sept niveaux : c'est la partition du *Shatchakranirûpana*, elle est sourcée,
elle est celle que Guénon commente, et elle est celle qui porte les ancrages
inter-registres validés. Rien n'est à corriger.

**Cela ouvre en revanche une question de modèle, à soumettre à Sidy** : les
centres surnuméraires sont-ils
1. *hors périmètre* (le registre déclare une partition et une seule) —
   position actuelle par défaut, tenable ;
2. un **second registre `hindouisme/tantra-etendu`**, `axe: parallele`, à onze
   niveaux, ancré au premier par des ancrages intra-traditionnels ; ou
3. une **strate d'annotation** sur les niveaux existants (des *entre-deux*
   affichés à la demande, sans rang propre) ?

Mon relevé de forme, sans verdict (Cmd 12) : l'option 2 est celle que le
générateur sait déjà faire — `axe: parallele` existe, la garde inter-registres
existe — mais elle exigerait des ancrages entre deux registres d'une même
tradition, cas que la garde traite aujourd'hui comme inter-registres et donc
comme exigeant une fiche `discernement/`. Or il ne s'agirait pas ici d'un
rapprochement entre traditions (Cmd 3), mais d'un rapport interne à une
tradition. **La garde serait donc à raffiner avant, pas après.** L'option 3 est
la moins coûteuse et ne force aucune décision. L'option 1 est le statu quo et
n'est pas fausse — elle est seulement muette.

Aucune de ces trois options n'est implémentée. Aucun champ n'est ajouté au
YAML. Ce paragraphe est un relevé de bifurcation, à trancher par Sidy.

### 7.4 Acquis annexe : la nomenclature fonctionnelle des canaux

Avalon nomme des nâdîs **par fonction** (*Manovahā*, véhicule de la conscience ;
*Ajñāvahā*, descendante et motrice ; les cinq sensorielles *Gandhavahā*,
*Rūpavahā*, *Shabdavahā*, *Rasavahā*, *Sparshavahā*). Cela donne au réseau, pour
la première fois au dépôt, des **arêtes qualifiées et orientées** — montantes
(sensorielles) et descendantes (motrices) — là où l'Instrument ne connaît
aujourd'hui que trois canaux et des ancrages non orientés.

C'est une piste de rendu réelle et non spéculative, mais **elle est adossée chez
Avalon à une réduction anatomique que Guénon écarte formellement**. Elle n'est
donc pas retenue en l'état : la retenir supposerait de séparer la nomenclature
(recevable) de son commentaire physiologique (non recevable), ce qui est un
travail de discernement, pas de rendu. **Non implémenté, consigné.**

### 7.5 Divergences table/planches

Deux écarts entre la table synoptique et les planches sont relevés dans la fiche
source (*Vishuddha* : triangle inscrit dans le cercle ; *Ājñā* : triangle
inversé là où la table ne donne rien). **Les données de l'Instrument ne sont pas
modifiées** : les champs `mandala` et `couleur_tattva` d'*Ājñā* restent vides.
Les combler d'après la planche serait juger que la planche prime la table —
un verdict, non un relevé. En attente de la parole de Sidy.

## 8. Les onze ancrages inter-registres, rendus — et ce que le rendu a révélé

Les onze ancrages verdictés étaient au manifeste depuis le 2026-08-29 **sans
être tracés** : la donnée portait onze joints que l'image ne montrait pas. Ils
sont désormais rendus, et le tracé lui-même a mis au jour trois faits qu'aucune
relecture n'avait relevés.

### 8.1 Ce qui est tracé

| Joints | Registres | État | Rendu |
|---|---|---|---|
| 4 | *tasawwuf* (bandes de Présences, tronc akbarien) ↔ *vedanta* | établi | trait plein |
| 5 | *qabbalah* ↔ *hindouisme-tantra* | établi | trait plein |
| 2 | *qabbalah* ↔ *hindouisme-tantra* (Yesod, Malkhut) | **suggéré** | **pointillé + 🔍** |

Établi et suggéré ne sont jamais fondus (règle des manifestes §3) : trait plein
contre pointillé, marqueur plein contre marqueur creux, 🔍 dans l'intitulé. Les
deux joints suggérés portent la réserve d'interversion **posée par Guénon
lui-même**, et doivent se lire autrement que les cinq autres.

### 8.2 Fait révélé — la coïncidence est un constat, pas un défaut

En mode axe unifié, cinq des sept joints *qabbalah* ↔ *hindouisme-tantra*
deviennent des **segments nuls** : les deux registres comptent le même nombre de
niveaux et sont ancrés niveau pour niveau, donc leurs nœuds tombent au même
point de l'axe unique.

Ce n'était pas prévu, et c'est le rendu qui l'a montré. Ce n'est pas un défaut à
corriger : **le segment nul EST le constat de commensurabilité**. Il est donc
tenu visible par un marqueur, et l'info-bulle le nomme.

### 8.3 Fait révélé — l'interversion se voit, et se voit comme un croisement

Les deux joints suggérés Yesod↔Mūlādhāra et Malkhut↔Swādhishthāna **se
croisent** : leurs milieux tombaient exactement au même point. C'est la figure
géométrique de l'interversion que Guénon signale sans la trancher. Les deux
marqueurs glissent maintenant à 35 % et 65 % de leur ligne propre, de sorte que
le croisement se voie et que les deux joints restent consultables séparément.

### 8.4 Erreur de ma part, corrigée avant commit

J'avais d'abord qualifié les ancrages sur leur **longueur totale**, et écrit
qu'un ancrage resté oblique en mode unifié signalerait un décalage-**donnée**.
C'était faux, deux fois :

1. La longueur mêlait la hauteur (le niveau, qui signifie) et le report latéral
   (la colonne, qui signifie autre chose) — Hokhma↔Ājñā ressortait « oblique »
   alors que ses deux extrémités sont **à la même hauteur**, l'écart étant
   purement latéral.
2. Surtout : la hauteur d'un domaine de registre parallèle est une
   **répartition proportionnelle** sur l'étendue de l'axe — présentation,
   jamais correspondance — tandis qu'une bande de Présence est posée à ses
   **degrés déclarés**. Un ancrage entre les deux a donc une extrémité de
   donnée et une extrémité de présentation : **son obliquité est un artefact de
   convention de hauteur, et rien d'autre.** En faire un indice de
   décalage-donnée aurait installé au cœur du rendu exactement la confusion que
   la spec interdit.

La qualification se lit désormais sur Y (le niveau) et sur X (la colonne),
**jamais sur Z** — la profondeur étant le décalage-artefact que le mode unifié
résorbe. Le constat est donc identique dans les deux modes, ce qui est la
vérité : la correspondance de niveau ne dépend pas de l'angle de vue.

### 8.5 Apport du ch. XXI de *L'Homme et son devenir selon le Vêdânta*

Texte primaire déposé par Sidy le 2026-08-30
([[doctrinal/sources/guenon-homme-devenir-vedanta-ch21-artere-coronale-rayon-solaire]]).
Quatre conséquences pour l'Instrument, dont trois versées en donnée :

1. **Le ternaire des canaux reçoit un troisième registre : le temps.**
   *piṅgalā* = œil droit = Soleil = **futur** ; *iḍā* = œil gauche = Lune =
   **passé** ; *suṣumnā* = œil frontal de Shiva = **présent**. Champs `oeil` et
   `temps` versés, rendus dans l'info-bulle des canaux.
2. **La chaîne devient entièrement guénonienne.** Le dépôt tenait colonnes ↔
   yeux ↔ luminaires du côté kabbalistique ; le ch. XXI donne l'autre moitié —
   ces deux yeux sont ceux de *Vaishwânara*, en relation avec les deux *nâdîs*.
   Plus aucun maillon rapporté du dehors.
3. **L'axe reçoit un prolongement nommé.** « Le rayon qui est en connexion avec
   l'artère coronale est appelé **aussi** *sushumnâ*. » Le canal individuel et
   le rayon universel portent le même nom : la thèse de l'axe unique est
   attestée par la **nomenclature du texte**, non par notre déduction. Champ
   `prolongement_axial` versé, **descriptif seulement** — aucun degré ne lui est
   attribué, la tradition ne le situant pas sur une échelle.
4. **Le canal médian devient consultable.** *suṣumnā* n'était atteignable nulle
   part dans le rendu : elle n'est pas dessinée à part, puisqu'elle EST l'axe.
   Un marqueur discret la rend consultable sans ajouter une seconde ligne qui
   mentirait sur la géométrie. Sa hauteur est celle du registre entier, **non**
   celle des hélices : les canaux latéraux s'arrêtent à *Ājñā*, l'axe non.

### 8.6 ⚠️ Réserve doctrinale que ce chapitre impose à toute figuration verticale

Elle vise précisément l'illusion qu'une échelle dessinée produit, et elle doit
être portée par l'Instrument, non seulement par les fiches :

> « tous ces états, étant encore relatifs et conditionnés, **n'ont aucune commune
> mesure** avec celui qui est seul absolu et inconditionné ; […] **leur obtention
> ne rapproche aucunement l'être de son but dernier** » ; et la Délivrance
> « impliquera toujours une **discontinuité** […] quel que soit cet état, cette
> discontinuité ne sera **ni plus ni moins profonde** ».

Une série de degrés superposés ne se lit donc jamais comme un rapprochement
progressif de l'Inconditionné : le sommet d'une échelle reste du côté du
conditionné, et le passage est également discontinu **depuis n'importe quel
degré**. L'Instrument dessine des hauteurs ; il ne doit pas laisser croire
qu'elles mesurent une proximité.

**Non implémenté, consigné comme exigence de rendu** : la réserve est portée
par la fiche source, non encore par l'interface. Trois pistes, aucune tranchée —
(a) une mention permanente dans la légende ; (b) le halo du *Hāhūt* (degrés
non manifestés) marqué comme non commensurable au reste de l'échelle ; (c) une
rupture graphique explicite entre le conditionné et l'Inconditionné. Verdict à
Sidy.

---

*Déduction et proposition. Aucun ancrage nouveau déclaré, aucun verdict rendu.
La correction d'une erreur de méthode de ma part (§2.1, §2.3) est consignée
explicitement. Les attributions d'échelle du §4.2 sont des lectures soumises à
Sidy, non des constats.*
