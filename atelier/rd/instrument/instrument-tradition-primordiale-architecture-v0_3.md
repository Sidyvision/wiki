---
title: "Instrument de la Tradition Primordiale — Architecture Fondamentale (v0.3)"
type: projet
...
> `atelier/projets/instrument-tradition-primordiale-architecture-v0_3.md` vers `atelier/rd/instrument/instrument-tradition-primordiale-architecture-v0_3.md` (ouverture du pôle R&D,
...
> - *2026-07-01 (v0.3)* — **arbitrages de Sidy sur les questions ouvertes du §8** :
>   §8.1 (lentille *barzakh*) **validé** ; §8.2 (directions horizontales) **tranché** en
>   faveur des quatre angles astrologiques AS/DS/MC/FC à qualités élémentaires ;
>   §8.3 (Phase 0) tranché en session A3 du même jour (Three.js/WebGL, web mobile,
>   hébergement statique) ; §8.4 (vigilance apophatique continue) **validé
>   formellement**. §6 : générateur de manifeste (Phase 1) livré. §7 : convention
>   `degre_vertical` fixée (échelle des 38 degrés). §8 réécrit en conséquence.
...
vertical du Tasawwuf est désormais fixée par une fiche doctrinale dédiée,
[[doctrinal/symboles/hadarat-khams]] (les Cinq Présences divines), sourcée principalement
sur Burckhardt. L'ordre, du sommet vers la base : *Hāhūt → Lāhūt → Jabarūt → Malakūt →
...
consécutifs), mais **son application nommée à une paire précise de Présences reste une
inférence, non une citation littérale** — voir [[doctrinal/symboles/hadarat-khams]] pour le
détail de cette nuance. La traduction technique de ce principe est désormais **actée
dans son principe (validée par Sidy, 2026-07-01)** : le *barzakh* est rendu comme
**lentille de transition visuelle** entre degrés adjacents de l'axe — jamais comme un
...
**démarche parallèle au développement lui-même**, dès maintenant. La révision v0.2
en fut elle-même un exemple concret : l'écart entre l'architecture figée et les décisions
prises en session n'avait été repéré qu'a posteriori, sur question explicite — précisément le
type de dérive que cet onglet est censé signaler en continu. **Ce principe est validé
formellement par Sidy (2026-07-01)** : la vigilance apophatique se pratique en continu
dès maintenant (voir §8, arbitrage n°4 ; `02-` à réviser en conséquence).
...
**Générateur livré (2026-07-01, Phase 1)** : `atelier/rd/outillage/generer-manifeste.py`
(script déterministe, zéro LLM, validations bloquantes — dont le correctif §2 en refus
mécanique), alimenté par la déclaration applicative
`atelier/rd/instrument/instrument-donnees.yaml` (nœuds, degrés, ancrages). Spécification :
`atelier/rd/outillage/spec-generateur-manifeste.md`. Premiers ancrages `etabli` inscrits :
la convergence des 28 (*Futūḥāt* ch. 198).

---

## 7. État d'implémentation des degrés du Tasawwuf (Phase 2 — en cours)

La doctrine des cinq degrés est fixée ([[doctrinal/symboles/hadarat-khams]]) et la
géométrie de l'axe l'est aussi (spec technique des 38 degrés).

**Convention `degre_vertical` — actée (2026-07-01)** : la coordonnée verticale
...
[[doctrinal/symboles/table-28-degres-nafas-rahman]], une fois ses colonnes complétées
(voir §8, point de données n°1).

Restent en phase technique :
- la génération des 38 nœuds-degrés depuis la table complétée ;
- le rendu d'Al-Insān al-Kāmil : **proposition (non validée)** — filament vertical
  continu, parallèle et légèrement décalé de l'axe neutre du Principe, traversant
  *Lāhūt* → *Nāsūt*, distinct de la chaîne des degrés (cf. §3.4 : nœud à part) ;
- le détail optique de la lentille *barzakh* (§3.4, principe acté).

---

## 8. Questions ouvertes — arbitrages du 2026-07-01 et points restants

### Tranchées (Sidy, 2026-07-01)

1. **Le *barzakh* comme lentille de transition visuelle** — **VALIDÉ**. Voir §3.4
   pour la formulation actée ; détail optique en phase technique.
2. **Directions horizontales** — **TRANCHÉ** : les **quatre angles astrologiques
   AS / DS / MC / FC**, porteurs des **qualités élémentaires**, structurent les
   directions horizontales de la scène 3D (la piste des Noms Divins abstraits est
   écartée comme structure primaire ; les deux pistes ne s'excluant pas, un
   rattachement des Noms pourra être étudié ultérieurement, adossé aux angles).
   Sources : `fin-des-temps-modernes-ilm-al-nujum-bases-mahdi-rouge` + schéma
   manuscrit de Sidy. ⚠ **`to-source`** : l'appariement exact qualité ↔ angle
   (Sec/Humide/Chaud/Froid) doit être transcrit depuis le schéma manuscrit / la
   source avant toute implémentation — ne jamais le poser de mémoire.
3. **Moteur 3D et cible** — **TRANCHÉ** (session A3 du 2026-07-01) : Three.js/WebGL,
   web mobile d'abord (Safari iPad, installable), hébergement statique (le manifeste
   est un fichier livré avec l'app). Phase 0 close.
4. **Méthode (§5)** — **VALIDÉ formellement** : la vigilance apophatique est
   pratiquée **en continu dès maintenant**, parallèlement au développement, et non
   comme livrable de Phase 4. `02-instrument-feuille-de-route.md` à réviser en
   conséquence.
...
4. **Calcul astrologique multi-méthodes** (Phase 5) et **échéancier** : à spécifier
   séparément, inchangés.

---

## 9. Ce que ce document n'est pas

Cette révision fixe l'**architecture fondamentale** (axe, arbre unique, ancrage, bande
de sélection, couche de données, mécanisme de suggestion, onglet apophatique), la
**base doctrinale du déploiement vertical Tasawwuf**, la **pile technique**
(Three.js/WebGL, web mobile, statique), la **convention `degre_vertical`** et les
**directions horizontales** (quatre angles). Elle ne fixe toujours pas :
- les valeurs des degrés pour les 38 nœuds-degrés (données de la table à compléter) ;
- le rendu 3D exact d'Al-Insān al-Kāmil (proposition en attente) et de la lentille *barzakh* ;
- l'appariement qualité ↔ angle (transcription `to-source`) ;
- l'intégration du calcul astrologique multi-méthodes ;
- l'échéancier de développement.

Ces points relèvent de la phase de données et d'implémentation — y compris,
le cas échéant, après revue par le Gem René Guénon.

---

*Statut : architecture fondamentale (v0.3) — Phase 0 close, Phase 1 (générateur de
manifeste) livrée, implémentation 3D non commencée. Classée en `atelier/projets/`
par liens à sens unique vers `doctrinal/` (jamais l'inverse, conformément à la règle
d'étanchéité du circuit Atelier). Les v0.1 et v0.2 restent au dépôt comme jalons.*
